...
USAGES["test_button_pressed.sh"]="Usage: test_button_pressed.sh [--socket <path>]\nStream button events from the daemon (TAP/HOLD/LONGHOLD/RELEASED). CTRL+C to stop."
USAGES["paging_daemon"]="Usage: bin/paging_daemon [--config <path>] [--ulanzi-sock <path>] [--cache <dir>]\nSingle C paging daemon: subscribes to read-buttons and renders/sends pages via ulanzi_d200_daemon."
USAGES["launch_stack.sh"]="Usage: launch_stack.sh [--config <path>] [--byobu|--kill]\nManual launcher: starts ulanzi_d200_daemon + paging_daemon."
...
USAGES["reset_ulanzi_usb.sh"]="Usage: bin/reset_ulanzi_usb.sh [--wait <sec>] [--wait-sock <path>] [--vidpid <VID:PID>]\nReset USB device 2207:0019 via unbind/bind (requires root)."
